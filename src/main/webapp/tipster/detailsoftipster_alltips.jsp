...
<div class="container mt-5">
	 ...
	 <div class="row bg-dark mb-3">
	    ...
	    <img class="image-fluid rounded-circle mb-2" style="width:100px;height:100px" src="https://www.eagletip.com/images/tipsters/${requestScope.topstats[0][3]}.jpg" onerror="this.onerror=null;this.src='https://www.eagletip.com/images/tipsters/${requestScope.topstats[0][3]}.png'" />
	    <div></div>
	    <img class="image-fluid rounded-circle mb-2" style="width:20px;height:20px" src="https://www.eagletip.com/images/sport/${requestScope.topstats[0][4]}.svg" />
	    <h5 class="text-nowrap" style="color:#14805E"><em>${requestScope.topstats[0][3]}</em></h5>
	    </div>
	    </div>
        <span class="float-right"><a class="btn-btn-primary d-sm-none" data-toggle="collapse" data-target=".multi-collapse" aria-expanded="true" aria-controls="multi1 multi2"><img src="https://www.eagletip.com/downarrow.svg" style="width:30px;height:30px"/></a></span>  
	    </div>
	    ...
	    <span class="float-right" style="color:red"><a class="btn-btn-primary d-none d-sm-block d-md-none" data-toggle="collapse" data-target="#multi2"><img src="https://www.eagletip.com/downarrow.svg" style="width:30px;height:30px"/></a></span>
	    </div>
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  <li class="nav-item">
    <a class="nav-link" id="home-tab" href="https://www.eagletip.com/tipster/activetips?name=${requestScope.tipstername}" role="tab" aria-controls="home" aria-selected="false">Today's Tips</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" id="profile-tab" href="https://www.eagletip.com/tipster/stats?name=${requestScope.tipstername}" role="tab" aria-controls="profile" aria-selected="false">Stats</a>
  </li>
  <li class="nav-item">
    <a class="nav-link active" id="settings-tab" href="https://www.eagletip.com/tipster/tips?name=${requestScope.tipstername}" role="tab" aria-controls="settings" aria-selected="true">Latest Tips</a>
  </li>
</ul>
...
$(document).ready(function(){
	...
	<c:forEach items="${requestScope.tips}" var="item1">
	 $("#mytable tbody").append("<tr><td><img src='https://www.eagletip.com/images/sport/${item1[6]}.svg' style='width:20px;height:20px'/></td><td><a class='text-warning' href='https://www.eagletip.com/tip?id=${item1[0]}'>${item1[1]} vs ${item1[2]}</a></td><td class='timecolumn'>${item1[3]}</td><td>${item1[7]}</td><td>${item1[8]} ${item1[9]}</td><td class='oddscolumn'>${item1[10]}</td><td>${item1[11]}</td><td class='bookmakercolumn'>${item1[12]}</td><td><c:if test='${item1[13]=="won"}'><img src='https://www.eagletip.com/images/css/right.svg' style='width:20px;height:20px'/></c:if><c:if test='${item1[13]=="lost"}'><img src='https://www.eagletip.com/images/css/wrong.svg' style='width:20px;height:20px'/></c:if></td><td><span <c:if test='${item1[14] > 0}'>class='text-success'</c:if> <c:if test='${item1[14] < 0}'>class='text-danger'</c:if>>${item1[14]} </span> &nbsp;</td></tr>");
	</c:forEach>
...
if(w_width>=576 && w_width<768){
 ...
 $(".add_to_cart").click(function(){
	   ...
	   var items2=items;
		$.ajax({
			method:"POST",
			url:"https://www.eagletip.com/AddCartItem",
			data:{
				itemname:name,
			...
			success:function(response){
				...
				if(response==="error"){
					...
					$("#cartitems").append("<div class='row justify-content-between'><div class='col-xs-4'><img style='width:30px;height:30px' class='image-fluid rounded-circle mb-2' src='https://www.eagletip.com/images/tipsters/"+res.name+".jpg'/><p class='text-nowrap text-left'>"+res.name+"</p></div><div class='col-xs-2'><p><b>"+res.months+" month subscription</b></p></div><div class='col-xs-2'><span><b>"+parseFloat(convcurrency*parseInt(res.months)).toFixed(2)+"</b> </span> <img style='width:20px;height:20px' src='https://www.eagletip.com/images/css/currency/"+currency+".svg'/><button type='button' id='removesubscription' class='close' aria-label='Close'><span style='color:red;' aria-hidden='true'>&times;</span></button></div></div>");
 ...
 $("#next").prop("href","https://www.eagletip.com/tipster/tips?page="+(current+1)+"&name=${requestScope.topstats[0][3]}");
 $("#previous").prop("href","https://www.eagletip.com/tipster/tips?page="+(current-1)+"&name=${requestScope.topstats[0][3]}");
 $("#first").prop("href","https://www.eagletip.com/tipster/tips?page="+1+"&name=${requestScope.topstats[0][3]}");
 $("#last").prop("href","https://www.eagletip.com/tipster/tips?page="+(max-1)+"&name=${requestScope.topstats[0][3]}");
	...
	if("${requestScope.category}"=='free'){
		...
		$.ajax({
	    	method:"POST",
	    	url:"https://www.eagletip.com/CheckToDisableSubscribeButton",
	...
	$(".subscribebutton").click(function(){
			...
			if(!($(".subscribebutton").hasClass("subscribed"))){
    		    $.ajax({
    		    	method:"POST",
    		    	url:"https://www.eagletip.com/SubscribeFreeTipster",
    		    	data:{username:sessionuser,tipstername:"${requestScope.topstats[0][3]}",email:"${sessionScope.email}"},
    		    	success:function(){
			...
			//remove the subscription
				  ...
				  $.ajax({
	    		    	method:"POST",
	    		    	url:"https://www.eagletip.com/RemoveSubscribedFreeTipster",
	    		    	data:{username:sessionuser,tipstername:"${requestScope.topstats[0][3]}",email:"${sessionScope.email}"},
	    		    	success:function(res){
	    		    		console.log(res);
	    		    		if(res.trim()=='success'){
	...
	$("#pagination").html("");	
	   current=parseInt(myval);
				  ...
				  for(var i=init;i<init+size;i++){
					$("#pagination").append("<a id='"+i+"' class='btn btn-dark text-light' href='https://www.eagletip.com/tipster/tips?page="+i+"&name=${requestScope.topstats[0][3]}'>"+i+"</a>")
